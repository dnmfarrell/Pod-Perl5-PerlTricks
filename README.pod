...
Pod::PerlTricks - Perl 6 parser for PerlTricks pseudopod
...
The ToHTML and ToJSON action classes render these begin/end blocks as C<E<lt>preE<gt>> nodes, with a matching class attribute so they can be rendered differently.
...
For C<Pod::PerlTricks::ToHTML> This will be rendered in a div node with the class attribute C<title>, and added to the head node. For C<Pod::PerlTricks::ToJSON> article metadata directives are included as a key/pair value in the head key, but not rendered in the body.
...
G stands for GitHub. It should build a hyperlink to a Github repo or user page.
